...
setup_env_file() {
  ...
  if [ -f ".env.example" ]; then
    ...
    echo "# Environment variables for InfraUtilX EC2 with EBS Blueprint" > "$temp_env_file"
    echo "" >> "$temp_env_file"
    ...
    echo "INSTANCE_TYPE=t2.micro" >> "$temp_env_file"
    echo "EBS_SIZE=20" >> "$temp_env_file"
    echo "PROJECT_NAME=infrautilx-example" >> "$temp_env_file"
  ...
  if [ "$SKIP_PROMPTS" != "true" ]; then
    ...
    # Ask for EBS size
    read -p "Enter EBS volume size in GB [20]: " EBS_SIZE
    EBS_SIZE=${EBS_SIZE:-20}
    sed -i.bak "s/EBS_SIZE=.*/EBS_SIZE=$EBS_SIZE/" "$temp_env_file" && rm -f "$temp_env_file.bak"
    
    # Ask for project name
    read -p "Enter project name [infrautilx-example]: " PROJECT_NAME
    PROJECT_NAME=${PROJECT_NAME:-infrautilx-example}
...
if [ "$DESTROY" = true ]; then
  ...
  # Make the test_ssh.sh script executable
